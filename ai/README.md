...
This repository is configured as a **Claude Code skills repository** containing 9 comprehensive skills for Drupal development.
...
2. **Install dependencies** (if any):
   ```bash
   # No additional dependencies needed for skill development
   ```
...
1. **Update version numbers**:
   ...
   find claude-skills -name "SKILL.md" -exec sed -i.bak 's/version: [0-9]\+\.[0-9]\+\.[0-9]\+/version: 1.1.0-dev/' {} \;
   ```
...
2. **Update version for production**:
   ...
   find claude-skills -name "SKILL.md" -exec sed -i.bak 's/version: [0-9]\+\.[0-9]\+\.[0-9]\+-dev/version: 1.0.0/' {} \;

   # Clean up backup files
   find claude-skills -name "*.bak" -delete
   ```
...
4. Test thoroughly with a real Drupal site
