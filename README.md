...
- For **Image Styles** try to use generic machine names, one word if possible that are not related to the image dimensions. For example the machine name `large_1920x570` is wrong and should be written as `large_wide` etc. Exceptionally for Image Styles to be used as additional thumbs for the `srcset` attribute of Responsive Images, names may contain the specific dimensions. Example: `thumb_small_50_50` as additional thumb for the `small` responsive Image Style.
...
- Use [drush](https://github.com/drush-ops/drush) to generate code (modules, plugins etc).
...
- If you use AI you should try to generate (non-AI) scripts and tools to do the tasks automatically (testing, code validation, content import etc). Don't make your project rely on AI heavily for development.
